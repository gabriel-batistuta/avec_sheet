...
class DataBase():
    ...
    def insert_in_table(self, table_name, row:dict, errors:int):
        ...
        # adiciona um numero a uma chave repetida
...
def get_json_database(JSON:dict):
            ...
            for especific_report in report_categorie_value:
                for table_name, table_values in especific_report.items():
                    ...
                    for row in table_values:
                        print('--------')
                        for table_key, table_value in row.items():
                            row[table_key] = data_type_manager(table_key, table_value)
                            print(table_value)
...
if __name__ == '__main__':
    ...
    js = get_json_database(JSON)
    print(js)
    with open('database-avec.json','w') as file:
        file.write(json.dumps(js, indent=4, ensure_ascii=False))
